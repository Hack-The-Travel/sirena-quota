...
def get_s7_quota():
    gateway = 'https://noded1.production.webservices.amadeus.com/'
    wsap = ''  # secret
    user = ''  # secret
    office_id = ''  # secret
    password = ''  # secret
    ...
    nonce = get_nonce()
    password_digest = get_password_digest(nonce, timestamp.encode('ascii'), password)
    headers = {
  ...
  <AMA_SecurityHostedUser xmlns="http://xml.amadeus.com/2010/06/Security_v1">
    <UserID POS_Type="1" PseudoCityCode="{pseudo_city_code}" RequestorType="U" />
       ...
       <soapenv:Body>
        ...
        message_id=str(uuid.uuid4()), soap_action=soap_action, endpoint=endpoint,
        username=user,
        password=base64.b64encode(password_digest).decode('utf-8'),
        nonce=base64.b64encode(nonce).decode('utf-8'),
        created=timestamp,
        duty_code=duty_code, pseudo_city_code=office_id,
    )
    print(rq)
    r = requests.post(endpoint, headers=headers, data=rq)
    print(r.text)
